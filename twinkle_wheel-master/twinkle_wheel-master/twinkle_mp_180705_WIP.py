...
def main():
    ...
    # wheel 2, need to adjust threshold and contouring, jahn's wheel
    ...
    frames, frames_rand, intensity_ratio, pixel_intensity = get_frames(viewport_image, contour_image, frame_format="PIL")
    ...
    get_pixel_intensity_plot(pixel_intensity)
    # print(intensity_ratio)
...
def get_frames(norm_image, contour_image, frame_format="PIL"):
    ...
    for i in range(int(360/rotation_per_frame)):
        ...
        # pixel_intensity = np.zeros((image_viewport_w, image_viewport_h), np.uint8)
        ...
        for x_point in range(image_viewport_w):
            for y_point in range(image_viewport_h):
                ...
                if viewport_np[x_point, y_point] > 10:
            ...
            pixel_intensity_ratio = (int(point_intensity)/256.0) * 100
    # for i in range(int(360/rotation_per_frame)):
    #     degrees = int(i * rotation_per_frame)
    #     normalized_disk_image = get_disk_image(contour_image, degrees, contour_w, contour_h, image_viewport_w, image_viewport_h)
    #     binary_normalized_image = np.array(normalized_disk_image.convert('L'))
    ...
    #                     new_f_rand[random_x_point, random_y_point] = normalized_disk_image_array[x_point, y_point]
        ...
        # print(pixel_intensity_ratio)
        pixel_intensity.append(float('{0:.2f}'.format(pixel_intensity_ratio)))
    return frames, frames_rand, intensity_ratio, pixel_intensity
...
def generate_plot(base_image, frames, frames_rand):
    ...
    #     # print(int(current_frame[random_x_point, random_y_point])/255.0)
    #     # pixel_intensity.append(int(current_frame[random_x_point, random_y_point])/255.0)
    ...
    # # scatplot = plt.scatter(fig, update)
    # # animation = FuncAnimation(fig, update, interval = delay_in_ms, repeat = True)
    # plt.show()
    ...
    def update(frame_number):
        ...
        # print(int(current_frame[random_x_point, random_y_point])/255.0)
        # pix_density = int(current_frame[random_x_point, random_y_point])/255.0
        # pixel_intensity.append(pix_density)
        # print(current_frame[random_x_point, random_y_point])
        # Update the scatter collection with the new colors, size, and positions
        scat.set_edgecolors(fibers['color'])
        scat.set_facecolors(fibers['color'])
...
# def get_scatter_frames(base_image, frames, frames_rand):
#     plt.style.use('dark_background')
#     total_pixels_list = get_list_of_total_shown_pixels(base_image)
#     n_drops = len(total_pixels_list)
#     # Re-calculate because not a perfect square for plotting
...
def get_pixel_intensity_plot(pixel_intensity):
    y = pixel_intensity
    x = range(len(pixel_intensity))
    # x = [0,180,1]
    # for i in range(pixel_intensity):
    #     points = int(i)
    #     x.append(points)
    # for i in range(0, len(pixel_intensity)):
    # for i in len(pixel_intensity):
    #     x.append(i)
    # print(len(y))
    plt.figure(2)
    # print(len(x))
    ...
    plt.axes()
    # plt.ylim([0, 100])
    plt.title('Single Fiber Intensity Ratio')
    plt.xticks(np.arange(0,361, 180))
    plt.xlabel('rotation (deg)')
    plt.ylabel('intensity ratio (%)')
    plt.plot(x, y, color = 'b')
